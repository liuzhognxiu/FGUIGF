...
namespace MetaArea.Editor.BuildItem
{
    ...
    public class BuildItemContainerEditor : UnityEditor.Editor
    {

        ReorderableList reorderList;

        BuildItemContainer myListBlock
        {
            get { return ((BuildItemContainer) target); }
        }

        void OnEnable()
        {
            // create reoderable list and setup handlers
            reorderList = new ReorderableList(myListBlock.items, typeof(MetaArea.BuildItem), true, true, true, true);
        ...
        void DrawElement(Rect rect, int index, bool active, bool focused)
        {
            ...
            myListBlock.items[index] =
                (MetaArea.BuildItem) EditorGUI.ObjectField(r, myListBlock.items[index], typeof(MetaArea.BuildItem), false);
        ...
        void GenerateAllGhostsInList()
        {
            for (int i = 0; i < myListBlock.items.Count; i++)
            {
                EditorUtility.DisplayProgressBar("Processing",
                    "Generating ghosts: " + (i + 1) + "/" + myListBlock.items.Count,
                    (float) i + 1 / (float) myListBlock.items.Count);
