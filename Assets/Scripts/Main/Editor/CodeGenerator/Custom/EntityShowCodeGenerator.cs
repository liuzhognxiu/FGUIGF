    ...
    public class EntityShowCodeGenerator : CodeGeneratorBase
    {
        ...
        public override bool GenCode()
        {
            ...
            using (StreamWriter sw = new StreamWriter(GetCodePath()))
            {
                ...
                sw.WriteLine("using MetaArea.Await;");
                sw.WriteLine("");
                ...
                //显示实体的可等待扩展
                sw.WriteLine(
                    $"\t\tpublic static async ETTask<{m_EntityName}Logic> Show{m_EntityName}Async(this EntityComponent entityComponent,{m_EntityName}Data data)");
                sw.WriteLine("\t\t{");
                sw.WriteLine(
