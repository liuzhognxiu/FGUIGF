...
namespace DE.Editor.DataTableTools
{
    ...
    {
        private static void GenerateUIFormID(string datatableName)
        {
            if (datatableName == "UIForm")
            {
                UIFormIdCodeGenerator generator = new UIFormIdCodeGenerator();
                generator.GenCode(true);
            }
        }
        ...
        public static void GenerateDataTablesFromTxtNotFileSystem()
        {
            ...
            foreach (var dataTableName in HotfixDataTableConfig.DataTableNames)
            {
                ...
                GenerateUIFormID(dataTableName);
            }
        ...
        [MenuItem("Tools/DataTable/Generate Hotfix DataTables/Excel To Txt", priority = 32)]
        public static void ExcelToTxt()
        {
            if (!Directory.Exists(HotfixDataTableConfig.ExcelsFolder))
            {
                Debug.LogError($"{HotfixDataTableConfig.ExcelsFolder} is not exist!");
                return;
            }

            ExcelExtension.ExcelToTxt(HotfixDataTableConfig.ExcelsFolder);
